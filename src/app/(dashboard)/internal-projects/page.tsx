...
export default function InternalProjectsPage() {
  ...
  const atRiskProjects = filteredProjects.filter(p => p.status === 'client_review' || (p.targetDueDate && p.targetDueDate < Date.now() + 7 * 24 * 60 * 60 * 1000 && p.status !== 'complete')).length;
  const lateProjects = filteredProjects.filter(p => p.targetDueDate && p.targetDueDate < Date.now() && p.status !== 'complete').length;
    ...
    <>
      ...
      <div className="flex flex-1 flex-col gap-4 p-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-slate-900 dark:text-white">Internal Projects</h1>
            <p className="text-slate-600 dark:text-slate-300">Internal project tasks by project</p>
          </div>
          <div>
            <button
        ...
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
          <Card className="gap-3 py-3">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 py-2">
              <div className="text-xs font-medium">Total Internal Projects</div>
                        ...
                        <div className="flex items-center justify-between gap-2">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="inline-flex items-center gap-1">
                              ...
                              <span className="text-sm text-muted-foreground ml-0.5">{project.department?.name || 'Department'}</span>
                              <button
