...
export default function EditSprintPage() {
  ...
  useEffect(() => {
    if (sprint) {
      ...
      setClientId(sprint.clientId as any);
      setDepartmentId(sprint.departmentId as any);
      setStartDate(new Date(sprint.startDate).toISOString().slice(0, 10));
      setEndDate(new Date(sprint.endDate).toISOString().slice(0, 10));
    }
  }, [sprint]);
  ...
  return (
    <>
      ...
      <main className="flex flex-col gap-6 p-4 md:p-6">
        ...
        <Card>
          <CardHeader className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
            ...
            <div className="flex gap-2 self-end md:self-auto">
              ...
              <Button
                onClick={async () => {
                  if (!ready || !name.trim() || !startDate || !endDate || !sprint) return;
                  await updateSprint({
                    id: sprint._id,
                    name: name.trim(),
        ...
        {/* Footer removed; actions are in the card header to match create */}
      </main>
