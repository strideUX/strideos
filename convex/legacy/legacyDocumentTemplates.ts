...
export const getActiveTemplates = query({
  ...
  handler: async (ctx) => {
    ...
    return await ctx.db
      ...
      .withIndex('legacy_by_active', (q) => q.eq('isActive', true))
      .collect();
  },
});
...
export const getTemplate = query({
  args: { templateId: v.id('legacyDocumentTemplates') },
  handler: async (ctx, args) => {
    const user = await getCurrentUser(ctx);
    if (!user) {
      throw new Error('Authentication required');
...
export const createTemplate = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const templateId = await ctx.db.insert('legacyDocumentTemplates', {
      name: args.name,
...
export const initializeDefaultTemplates = mutation({
  ...
  handler: async (ctx) => {
    ...
    const existingTemplate = await ctx.db
      .query('legacyDocumentTemplates')
      .withIndex('legacy_by_document_type', (q) => q.eq('documentType', 'project_brief'))
...
export const updateTemplate = mutation({
  args: {
    templateId: v.id('legacyDocumentTemplates'),
