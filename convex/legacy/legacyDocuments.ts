...
export const createDocument = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const documentId = await ctx.db.insert('legacyDocuments', {
      title: args.title,
    ...
    if (args.templateId) {
      ...
      template = await ctx.db
        .query('legacyDocumentTemplates')
        .withIndex('legacy_by_document_type', (q) => q.eq('documentType', args.documentType))
    ...
    if (template && template.defaultSections.length > 0) {
      for (const sectionTemplate of template.defaultSections) {
        await ctx.db.insert('legacyDocumentSections', {
          documentId,
  ...
  handler: async (ctx, args) => {
    ...
    if (args.clientId) {
      documents = await ctx.db
        .query('legacyDocuments')
        .withIndex('legacy_by_client', (q) => q.eq('clientId', args.clientId!))
        .collect();
    } else if (args.departmentId) {
      documents = await ctx.db
        .query('legacyDocuments')
        .withIndex('legacy_by_department', (q) => q.eq('departmentId', args.departmentId!))
        .collect();
    } else {
      ...
      if (user.role === 'client' && user.clientId) {
        documents = await ctx.db
          .query('legacyDocuments')
          .withIndex('legacy_by_client', (q) => q.eq('clientId', user.clientId!))
          .collect();
      } else {
        // For other roles, show all documents they have access to
        documents = await ctx.db.query('legacyDocuments').collect();
      }
    }
